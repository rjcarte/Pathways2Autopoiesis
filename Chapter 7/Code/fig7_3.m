% Information Niche model simulations
%
% (c) 2018 R.J. Carter
% School of Chemistry
...
% Load automata information e.g. interaction matrix ('G')
load('jointG')
...
M = zeros(n,n,'single'); % Create the lattice
neighbourhood = 9; % Moore neighbourhood interactions
phi = 0; % Influx rate of randomly generated automata into population
cMix = 5; % No. of automaton mixed on each time step (diffusive mixing)
v = 100; % Distance from which automaton are mixed (diffusive mixing)
phiAlt = 0; % Set to '1' to restrict influxing machines to subset (only used to generate Niche F and Niche Y)
E(1,:) = [phi cMix v 1]; % Program in the timing of environment parameters (syntax: [phi cMix v z])

% POPULATION PARAMETERS
INITTYPE = 1888; % Initial number of automata types in the population. This is FIXED.
histSamplingRate = 50;
popDynamics=zeros(Z/histSamplingRate,INITTYPE,'single'); % Population dynamics history
N = INITTYPE * 50; % Population size
MAXTYPES = 1:INITTYPE; % Population diversity
MSamplingRate = Z;
Mhistory=cell(1,Z/MSamplingRate);
...
while z < Z
    ...
    if mod(z,histSamplingRate) == 0
        popDynamics(histCounter,:)=norm_freq;
        histCounter=histCounter+1;
    end
    
    if mod(z,MSamplingRate) == 0
        Mhistory{Mcounter} = M;
        Mcounter = Mcounter + 1;
    end
