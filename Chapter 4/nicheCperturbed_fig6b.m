...
% NOTES:
%
% Simulation demonstrates irreversible perturbation of niche A to form niche X
%
...
cMix = 0; % No. of automaton mixed on each time step (diffusive mixing)
v = 0; % Distance from which automaton are mixed (diffusive mixing)
phiAlt = 0; % Set to '1' to restrict influxing machines to subset (only used to generate Niche F and Niche Y)
E(1,:) = [0 0 0 1]; % Program in the timing of environment parameters (syntax: [phi cMix v z])
%E(2,:) = [0.08 0 0 1e6]; % Perturb niche A at z=1e6 by removing all diffusive transfer
%E(3,:) = [0 5 10 5e6]; % Remove perturbation and set environment back to original settings
...
while z < Z
    ...
    % Calculate complexity of the lattice
    dM = dzip(M);
    dd = whos('dM');
    dd = dd.bytes;
    complexityM(z) = log2(dd); 
